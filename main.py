...
async def handle_button(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            await query.edit_message_text(f"📤 Uploading *{title}* ({size_str})...", parse_mode="Markdown")
            MAX = 50 * 1024 * 1024
            with open(filepath, 'rb') as f:
                if size > MAX:
                    await query.edit_message_text("❌ File too large (>50MB). Try 480p or 360p.")
                    return
                elif mode == "a" or ext in ['.mp3','.m4a']:
                    await query.message.reply_audio(audio=f, title=title,
                        performer=info.get('uploader',''),
                        caption=f"🎵 {title}\n_via Media Bot_", parse_mode="Markdown")
                    ...
                    await query.message.reply_video(video=f, supports_streaming=True,
                        caption=f"🎬 {title}\n_via Media Bot_", parse_mode="Markdown")
            await query.edit_message_text(f"✅ Done! *{title}* sent! 🎉", parse_mode="Markdown")
    except Exception as e:
        err = str(e)
        if "Sign in" in err or "bot" in err.lower():
